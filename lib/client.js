...
`
     ...
     * 左栏入口。
     *
     * 直接由官方 `sidebar.footer.action` 插槽渲染，不做任何 DOM 注入：
     * 不查询、不插入、不搬移别人的节点，也不观察 DOM 变化。
     *
     * 这样做是刻意的。侧边栏里已有插件（如 MCP 连接器）用 MutationObserver
     * 维护自己的位置不变式；若本插件也去抢同一块 DOM，两边的 observer 会
     * 互相把对方搬走，形成无限微任务循环，把渲染进程 CPU 打满直至崩溃。
     * 把位置完全交给插槽宿主决定，就不存在可争抢的共享状态。
     */
    function ToolboxEntry(props) {
      ...
      return h(
        'div',
        { className: 'dshToolboxEntry', 'data-wide': wide },
        h(
          ActionButton,
          {
            variant: 'ghost',
